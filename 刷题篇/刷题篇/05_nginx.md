...
```ini
    #当客户端访问www.yuchaoit.cn,监听端口号为80,直接跳转到data/www目录下文件
    server {
        listen       80;
    ...
    #当客户端访问www.yuchaoit.cn,监听端口号为80,直接跳转到data/bbs目录下文件
    server {
        listen       80;
...
```ini
    #当客户端访问www.yuchaoit.cn,监听端口号为8080,直接跳转到data/www目录下文件
    server {
        listen       8080;
    ...
    #当客户端访问www.yuchaoit.cn,监听端口号为80直接跳转到真实ip服务器地址 127.0.0.1:8080
    server {
        listen       80;
        server_name  www.yuchaoit.cn;
        location / {
...
- 漏桶算法是网络世界中流量整形或速率限制时经常使用的一种算法，它的主要目的是控制数据注入到网络的速率，平滑网络上的突发流量。漏桶算法提供了一种机制，通过它，突发流量可以被整形以便为网络提供一个稳定的流量。也就是我们刚才所讲的情况。漏桶算法提供的机制实际上就是刚才的案例：
  ...
  ![在这里插入图片描述](pic/17172646dbb8b696~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
![在这里插入图片描述](pic/17172646dbc20c88~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
